...
from paintbynumbers import generate_paint_by_numbers

@app.route("/paintbynumbers", methods=["POST"])
def paint_by_numbers_route():
    if "image" not in request.files:
        return jsonify({"error": "No image provided"}), 400
    try:
        file = request.files["image"]
        image_path = "/tmp/temp_input.jpg"
        file.save(image_path)

        n_colors = int(request.form.get("colors", 24))
        painted, canvas = generate_paint_by_numbers(image_path, n_colors)
        ...
        with ZipFile(zip_io, 'w') as zf:
            zf.writestr("canvas.png", canvas_io.getvalue())
            zf.writestr("painted.png", painted_io.getvalue())

        zip_io.seek(0)
        return send_file(zip_io, mimetype='application/zip', download_name="paintbynumbers.zip")
